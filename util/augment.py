...
class AugmentPipe(torch.nn.Module):
    def __init__(self, init_p=0,
        ...
        brightness=0, contrast=0, lumaflip=0, hue=0, saturation=0, brightness_std=0.25, contrast_std=0.1, hue_max=1, saturation_std=0.25,
        ...
        self.Hz_geom = upfirdn2d.setup_filter(wavelets['sym6'])
    ...
    def forward(self, images, masks=None, debug_percentile=None, args=None, debug=False):
        ...
        if G_inv is not I_3:
            ...
            # if debug and torch.sum(i) > 0:
            #     args.file_log.debug(f"[{args.idx} , {args.debug_str}] geometric: {G_inv}")

            # if debug and torch.any(images.isnan()):
            #             print("GEOM is NaN")
        ...
        if self.imgfilter > 0:
            ...
            Hz_prime = g @ self.Hz_fbank.to(device)                                    # [batch, tap]
            Hz_prime = Hz_prime.unsqueeze(1).repeat([1, num_channels, 1])   # [batch, channels, tap]
            Hz_prime = Hz_prime.reshape([batch_size * num_channels, 1, -1]).to(device) # [batch * channels, 1, tap]
            ...
            if debug and torch.sum(i) > 0:
                args.file_log.debug(f"[{args.idx} , {args.debug_str}]")
